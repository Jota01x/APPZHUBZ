...
import { LayoutDashboard, Globe, Puzzle, Settings, LogOut, PanelLeftClose, PanelLeftOpen, MessageSquare, User, X, Check, Users, ShieldAlert } from 'lucide-react';
  ...
  return (
    <>
      ...
      `}>
        ...
        <nav className="space-y-2 flex-grow overflow-y-auto custom-scrollbar font-sans">
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => {
              ...
              className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-sm font-bold relative group ${
                currentView === item.id 
            ...
            >
              ...
              {isCollapsed && (
                <div className="absolute left-full ml-4 px-3 py-1.5 bg-zinc-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 border border-zinc-700 shadow-xl uppercase font-sans font-bold tracking-wider">
                  {item.label}
                </div>
              )}
            </button>
        ...
        <div className="mt-auto space-y-2 pt-6 border-t border-zinc-800 font-sans">
          <button 
            onClick={handleLogout}
            className={`w-full flex items-center gap-3 px-3 py-3 text-zinc-400 hover:bg-red-900/20 hover:text-red-400 rounded-xl transition-all text-sm font-bold ${isCollapsed ? 'justify-center' : ''}`}
          >
            <LogOut size={20} />
            {!isCollapsed && <span className="uppercase tracking-wider">Sair da Conta</span>}
          </button>
        </div>
      </aside>
    </>
